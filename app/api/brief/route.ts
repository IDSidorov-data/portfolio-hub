...
export async function POST(req: NextRequest) {
  ...
  // honeypot/time-before RL, как в ТЗ
  ...
  // валидация (сервер — источник истины)
  ...
  // идемпотентность (до троттлинга): одинаковую полезную нагрузку "глотаем"
  const sigParts = [
    name,
    telegram,
    email,
  ...
  // если есть файл — отправляем вторым сообщением
  if (file) {
    ...
    const sendDoc = await fetch(`${base}/sendDocument`, { method: "POST", body: fd });
    if (!sendDoc.ok) {
      // не валим заявку целиком; логировать PII нельзя — просто завершим
    }
  }

  // готовим ответ + куки (RL и SIG)
