...
const STOPWORDS = ["крипт", "ставк", "заработай", "заработок", "xxx", "18+", "free money"];
...
const TG_TOPIC_ID_NUMBER = TG_TOPIC_ID ? Number(TG_TOPIC_ID) : NaN;
...
function capsRatio(s: string) {
  ...
  if (!letters) return 0;
  const caps = (s.match(/[A-ZА-ЯЁ]/g) || []).length;
  return caps / letters;
}
...
function corsHeaders(req: NextRequest): HeadersInit | undefined {
...
export async function POST(req: NextRequest) {
  ...
  const normalized = about.toLowerCase();
  if (linksCount > 3 || capsRatio(about) > 0.8 || STOPWORDS.some((word) => normalized.includes(word))) {
    return NextResponse.json(
  ...
  const tgLink = telegram
    ? `<a href="https://t.me/${escHtml(telegram)}">@${escHtml(telegram)}</a>`
    : "";
  ...
  if (Number.isFinite(TG_TOPIC_ID_NUMBER)) {
    msgBody.message_thread_id = TG_TOPIC_ID_NUMBER;
  }

  const sendMessage = await fetch(`${base}/sendMessage`, {
    method: "POST",
  ...
  if (file) {
    ...
    if (Number.isFinite(TG_TOPIC_ID_NUMBER)) {
      fd.set("message_thread_id", String(TG_TOPIC_ID_NUMBER));
    }
  ...
  res.cookies.set("brief_rl", b64enc(String(Date.now())), {
    httpOnly: true,
    secure: true,
    sameSite: "strict",
    maxAge: 60,
  ...
  res.cookies.set("brief_sig", signature, {
    httpOnly: true,
    secure: true,
    sameSite: "strict",
    maxAge: 300,
